...
import warsztaty.java.ailleronLab.model.RootObject;
...
public class CountryService {
    ...
    public RestResponse getCountryFromRestApi(final String code){
        ...
        if(rootObjectEntity.getStatusCode().is2xxSuccessful()){
            RestResponse response = rootObjectEntity.getBody().getRestResponse();
            response.setSayHello(messageSource.getMessage("say.hello", null, Locale.forLanguageTag(code)));
